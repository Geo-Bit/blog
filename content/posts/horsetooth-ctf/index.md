---
title: "Building a CTF Web Application: Lessons from Horsetooth Liquidators"
...
<img src="logo.png" alt="Horsetooth Liquidators Logo" style="width:200px; display: block; margin-left: auto; margin-right: auto;"/>

# Intro
The Horsetooth Liquidators project is something I put together as part of my involvement with NoCo Hackers, a local security meetup. We ran our CTF event for the second time, and with a bigger crowd expected, I thought it was a good chance to try building a custom CTF platform.
...
We designed the event around a fictional Fort Collins business that needed a security check-up. Participants took on the role of security engineers, assessing the business's security and submitting flags in the CTF. It was all about crafting a unique experience for everyone involved. Checkout the original prompt via the [GitHub project](https://github.com/horsetooth-ctf/horsetooth-ctf).

Oh, and in case you were wondering, Horsetooth is in reference to the [Horsetooth Rock](https://www.coloradoan.com/story/life/2016/05/08/fleming-legend-horsetooth-rock/84124710/) (and Horsetooth Reservoir) here in Fort Collins, CO:
...
{{< carousel images="{homepage.png,inventory.png,404-not-found.png,chat.png}" >}}
<br>
...
- I didn't want to limit the tooling people could use for the CTF, so I had to design the infrastructure to be able to handle brute forcing tools like dirbuster and hydra as well as other tools like sqlmap and Burpsuite. Fortunately, we did not encounter any performance degradation or disruptions/outages, and the infrastructure scaled and held up well overall.
- Securing the flags from players was a challenge, which meant that I needed to prioritize adequate authentication/authorization between the frontend and backend.
...
2. Virtual Machines are no fun. For a moment, I was going to pivot to using IaaS/VMs so I could retain my Docker Compose setup, but I quickly realized how much of a pain reployment was going to be, and I'm glad I went serverless. 
...
### More Data 
I would have liked to invest more into telemetry and observability data. I was very curious about how people were interacting with and "poking" at the application. Google Cloud actually offers decent analytics, telemetry, and observability services (ie. Google Cloud Monitoring, Google Cloud Logging, and Google Cloud Trace). I would have liked to use these services to achieve better visibility and understanding of user interactions and behaviors during the CTF event but unfortunately this feel to the wayside. 

## CTF Design Insights
### Flag Validation and Security
...
Looking at the correlation of # of points to % solved, I feel like I did a decent job balancing the challenges with the exception of Iventory Overflow, which no one solved unfortunately and was supposed to be more of a "Medium" difficulty challenge.
...
I noticed that because of the contextual limitations, it really requires the developer to know be able to identify deviations and be familiar enough with web development (or general software development) to get the AI back on track. While the fixes typically involve clever prompting, I did have to revert to previous commits at times.
